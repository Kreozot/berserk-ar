 ...
 * card's perspective in the camera frame.
 */
...
export function normalizeCardPerspective(source: Mat, corners: Quadrilateral): Mat {
  ...
  const sourcePoints = PointVector.create();
  const destinationPoints = PointVector.create();
  ...
  const sourcePointObjects = [
    Point.create(corners[0].x, corners[0].y),
    Point.create(corners[1].x, corners[1].y),
    Point.create(corners[2].x, corners[2].y),
    Point.create(corners[3].x, corners[3].y),
  ] as const;
  const destinationPointObjects = [
    Point.create(0, 0),
    Point.create(NORMALIZED_CARD_WIDTH - 1, 0),
    Point.create(NORMALIZED_CARD_WIDTH - 1, NORMALIZED_CARD_HEIGHT - 1),
    Point.create(0, NORMALIZED_CARD_HEIGHT - 1),
  ] as const;
