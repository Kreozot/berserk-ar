...
export function normalizeCardPerspective(source: Mat, corners: Quadrilateral): Mat {
  ...
  const sourcePointObjects = [
    Point2f.create(corners[0].x, corners[0].y),
    Point2f.create(corners[1].x, corners[1].y),
    Point2f.create(corners[2].x, corners[2].y),
    Point2f.create(corners[3].x, corners[3].y),
  ] as const;
